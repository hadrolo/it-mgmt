...
class TokenFactory
{
    ...
    {
        $token = [
            "exp" => time() + FRAMEWORK['AUTH']['ACCESS_TOKEN_TIME'],
            "data" => [
                'UID' => $uid,
            ]
        ];
        ...
        $refresh_token = [
            "exp" => time() + FRAMEWORK['AUTH']['REFRESH_TOKEN_TIME'],
            "data" => [
                'UID' => $uid,
            ]
        ];

        $aToken = JWT::encode($token, FRAMEWORK['AUTH']['JWT_KEY']);
        $rToken = JWT::encode($refresh_token, FRAMEWORK['AUTH']['JWT_KEY']);
    ...
    {
        try {
            $decoded = JWT::decode($refreshToken, FRAMEWORK['AUTH']['JWT_KEY'], ['HS256']);
